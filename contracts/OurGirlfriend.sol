...
contract OurGirlfriend is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function startARelationship() public payable nonReentrant { 
        require(
            msg.value == PRICE,
            "It costs 0.07 ETH to start a relationship"
        );
        require(
            totalSupply() < MAX_SUPPLY,
            "Our GF is at max social capacity."
        );
        ...
        mintRevenue += PRICE; 
        _safeMint(_msgSender(), totalSupply()); // _msgSender() => MultiMint contract address
    }
    ...
    function relationshipStatus(uint256 tokenId)
        public
        view
    ...
    function relationshipLevel(uint256 tokenId)
        internal
        view
        returns (string memory)
    {
        ...
        returns (string memory)
    {
                    ...
                    abi.encodePacked(
                        ...
                        Base64.encode(
                            bytes(
                                string(
                                    abi.encodePacked(
                                        '{"name": "Asami (v0.1)`s Heart", "description": "Asami has found her soulmate.", "animation_url": "https://assets.idolresearchersapi.com/ASAMI-HEART.mp4", "attributes": [ { "trait_type": "Feels", "value": "Fulfilled" } ], "image": "https://assets.idolresearchersapi.com/ASAMI-HEART.png"}'
                                    )
                                )
                            )
                        )
        ...
        if (tokenId < 10) {
            paddedTokenId = string(
                abi.encodePacked("000", Strings.toString(tokenId))
            );
        } else if (tokenId < 100) {
            paddedTokenId = string(
                abi.encodePacked("00", Strings.toString(tokenId))
            );
        } else if (tokenId < 1000) {
            paddedTokenId = string(
                abi.encodePacked("0", Strings.toString(tokenId))
            );
        }
        ...
        string memory imageUrl = string(
            abi.encodePacked(
                "https://assets.idolresearchersapi.com/ASAMI-",
                paddedTokenId,
                "-",
                relationshipLevel(tokenId),
                ".svg"
            )
        );
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '" }, { "trait_type": "Auspicious Number", "value": ',
                                    Strings.toString((tokenId % 7) + 1),
                                    ' }, { "trait_type": "Zodiac", "value": "Scorpio" }, { "trait_type": "Anon Let`s", "value": "',
                                    HOBBIES[rand % HOBBIES.length],
                                    '" } ], "image": "',
                                    imageUrl,
                                    '"}'
                                )
